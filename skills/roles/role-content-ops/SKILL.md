...
| 本角色 `references/role-memory.md`（无限制） | Obsidian目录结构变更、内容策略决策、平台规则变化、运营经验 | `skill_manage` 工具 |

**更新规则：**
- 对话中产生的**运营领域知识**（如平台规则变化、内容策略调整、Obsidian目录重组）→ 通过 `skill_manage(action='patch', name='role-content-ops', file_path='references/role-memory.md', ...)` 更新
- **全局性事实**→ 通过 `memory` 工具更新
- 读取角色专属记忆：`skill_view(name='role-content-ops', file_path='references/role-memory.md')`
