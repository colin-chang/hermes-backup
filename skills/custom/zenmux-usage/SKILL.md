...
version: 1.1.0
author: ZenMux (adapted for Hermes Agent)
license: MIT
platforms: [linux, macos]
metadata:
  hermes:
    tags: [zenmux, usage, quota, billing, statistics, api]
    related_skills: [zenmux-integration]
---
...
This skill provides structured access to all seven ZenMux Management API endpoints — subscription details, Flow rate, PAYG balance, per-generation cost breakdown, usage timeseries, model leaderboards, and provider market share. All calls use `curl` + `jq` over the terminal, and results are formatted into human-readable tables.
...
This skill needs `ZENMUX_MANAGEMENT_KEY` available. In Hermes, the preferred way is to add it to Hermes's `.env` file:

```bash
# ~/.hermes/.env (or wherever Hermes reads env from)
ZENMUX_MANAGEMENT_KEY=sk-mgmt-...
```

### Step 0 — Verify the Management Key
...
- **`set`** → proceed to Step 1.
- **empty** → tell the user the key is missing. Offer:
  1. Add `ZENMUX_MANAGEMENT_KEY=sk-mgmt-...` to `~/.hermes/.env` and restart gateway
  2. Or point them to https://zenmux.ai/platform/management to create the key
...
1. **ZENMUX_MANAGEMENT_KEY not in scope**: Hermes's terminal may not inherit from interactive shell. Always verify with `echo "${ZENMUX_MANAGEMENT_KEY:+set}"` first. If missing, direct user to `~/.hermes/.env`.
2. **60-bucket limit**: Users often ask for "last 90 days" with `bucket_width=1d` → 90 buckets → 400 error. Auto-detect and propose `1w` instead.
3. **ISO-week snap**: `starting_at=2026-05-15` (Friday) + `bucket_width=1w` → server echoes `2026-05-11` (Monday). Always read echoed dates, not request dates.
4. **T-1 lag**: "Show today's usage" will return empty. Explain and offer yesterday's data.
5. **Subscription Detail monthly usage is missing**: Monthly quota shows max only (no real-time usage). This is expected API behavior — don't interpret as broken.

## Verification Checklist

- [ ] `ZENMUX_MANAGEMENT_KEY` confirmed set before API calls
- [ ] Correct endpoint chosen per user intent table
- [ ] Statistics params validated (metric, bucket_width, date range ≤ 60 buckets)
- [ ] Response parsed with `jq` and formatted per section templates
- [ ] Echoed dates from response used (not request dates) when reporting
- [ ] 80%+ quota thresholds highlighted as warnings
- [ ] `__others__` rendered as "Others" without rank number
