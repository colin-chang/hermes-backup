...
- Looking up ZenMux model availability, pricing, or capabilities (see `references/model-discovery.md`)
...
- Management API Key: ✅ **已配置** — `ZENMUX_MANAGEMENT_KEY` 已在 `.env` 中设置 (`sk-mg-v1-...`)
...
**前置条件：** Management API Key 已配置（`ZENMUX_MANAGEMENT_KEY` 已在 `~/.hermes/.env` 中设置）。

## Pitfalls

1. **Billing data delay**: Usage/billing fields appear 3-5 minutes after request completion.
...
4. **Vertex Monitor SSE streaming (⚠️ Hermes 集成关键)**: Hermes 全局 `streaming: true` 意味着所有请求带 `stream: true`。Vertex Monitor 代理若返回 `JSONResponse` 而非 SSE 格式（`text/event-stream`），Hermes 流式客户端解析失败 → "Empty response" → 不触发自动降级（Hermes 只在收到错误响应时降级）。**修复见 `references/vertex-monitor-streaming.md`，诊断模板可直接复用。**

## References

- `references/management-api.md` — 管理 API 端点详情
- `references/model-discovery.md` — 模型发现与定价（从 `zenmux.ai/models` 页面抓取 JSON-LD，获取所有模型的价格/能力）
- `references/vertex-monitor-streaming.md` — Vertex Monitor SSE 流式兼容修复（诊断步骤 + 修复方案 + 通用模式）
- `references/claude-code-model-tiers.md` — Claude Code 模型层级配置（ANTHROPIC_DEFAULT_*_MODEL 环境变量、版本要求、升级流程）

## Related Skills

- **`codex-third-party-providers`** — Codex + ZenMux/third-party provider 配置、模型发现排障、CC Switch 交互。当 Codex Desktop 无法切换模型、弹登录框、或模型目录不加载时使用。
