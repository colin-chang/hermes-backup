...
- **v3.25.0** (2026-05-24): **P50（评论→正文合并）已上线**。`stream_consumer.py:552-556` 的 `_send_commentary()` 独立消息路径改为累积到流式缓冲区，评论与正文合并为同一条消息。碎片从 N 条降为 2-3 条。已注册 `hermes-patches.sh` patch #14。详见 `hermes-agent` skill 的 `references/mattermost-streaming-fragmentation.md`。
