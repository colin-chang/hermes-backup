...
- **v3.14.0** (2026-05-22): `references/hermes-cron-pitfalls.md` 新增 P45（三层安全扫描架构）；P41/P43 更新为 execute_code 优先（cron `code_execution` toolset 已修正）；P44 确认"不自动触发"纪律。 → `history=0` → 用户感知为「串台」）。**非插件 Bug，Gateway 会话持久化问题**：Hermes Gateway 重启后，内存中会话的 conversation history 丢失。下次用户在同一 Thread 发消息时，session 命中但 `history=0`，Agent 不记得之前的对话内容。用户体感是「上下文错了 / 串台了」。诊断：检查重启时间戳与下一次 `conversation turn` 的 `history=N` 值——若 N=0 且重启发生在两次对话之间，则是会话丢失而非路由 Bug。详见 `references/gateway-restart-session-loss.md`。
- **v3.15.0** (2026-05-22): 新增 **P40-P44**（Hermes Cron 定时任务排障）：`enabled_toolsets` 用 toolset 名而非 tool 名（P40）、terminal `python3 -c` 触发安检 vs Python HEREDOC 绕过（P41）、运行时扫描器扫 Skill 全文（P42）、子代理 iMessage 发送模板（P43）、禁止自动触发 cron（P44）。详见 `references/hermes-cron-pitfalls.md`。
