...
文档迭代（小修改 → 审阅 → 调整）时，不要生成多条提交。使用 `amend + force push` 保持一条干净提交：

```bash
git add <files>
git commit --amend -m "..."   # 合并到上一次提交
git push --force
```

## 小贴士区块规范

当需要添加部署相关的补充信息（如 Docker 常见问题、网络配置提示），不要创建独立的大章节。应以 **blockquote 小贴士**形式放在「项目结构」之后、「许可」之前：
...
**规则：**
- 每个 tip 一行，问号开头 + 解决方案 + 原因（不超过一行）
- 不放代码块，不放子标题，不分「表现/原因/修复」小节
- 适用条件：内容 ≤ 5 条、每条能用一句话说清原因 → 用小贴士；内容 > 5 条或每条需要多段解释 → 用独立章节
- 中英文版本同步添加
...
- [ ] 小贴士区块（如有）放在项目结构之后、许可之前，blockquote 格式，每条 ≤ 一行
