...
description: "定义信息获取与浏览器操作的工具优先级策略：Web Search → Dokobot 降级链，有 Site Adapter 优先用 OpenCLI site CLI，通用交互用 Hermes browser_* 工具。"
version: 1.5.0
metadata:
  hermes:
...
**核心原则：有 Site Adapter 的平台 → OpenCLI site CLI 优先；通用交互 → Hermes browser_* 工具**

| 优先级 | 工具 | 适用场景 |
|--------|------|----------|
| 1️⃣ 首选 | `terminal("opencli <site> <command>")` | 目标平台有 OpenCLI Site Adapter 时，一条 CLI 直接调平台 API（100+ 站点），效率远超手动浏览器操作 |
...
| 优先级 | 模式 | 登录态 | 配置方式 |
|--------|------|--------|----------|
| 1️⃣ | **OpenCLI Browser Bridge** | ✅ 实时共享你现有 Chrome 登录态 | 安装扩展 → `opencli doctor` 验证 |
| 2️⃣ | `/browser connect` 直连主 Chrome | ✅ 实时共享 | Hermes 会话中执行 `/browser connect` |
| 3️⃣ | 默认 agent-browser（Camofox） | ❌ 独立环境 | 无需配置，Hermes 默认后端 |

**推荐模式 1️⃣**（OpenCLI Browser Bridge）的优势：
- 直接复用你现有 Chrome 所有登录态（零配置，零同步）
- 无需独立 Chrome 实例，无需 profile 同步脚本
- 无授权弹窗（与 `/browser connect` 的关键区别）
- 安装验证一条命令：`npm install -g @jackwener/opencli` + Chrome Web Store 扩展 + `opencli doctor`

> 详见 `references/opencli-integration.md`。
...
### OpenCLI site CLI（补充 — 平台数据获取）

OpenCLI 的核心差异化价值是 **100+ 站点的 Site Adapter 系统**，用真实浏览器的登录态直接调用网站 API：

```bash
opencli twitter search "AI agent" -f json
opencli zhihu hot -f json
opencli xueqiu stock "AAPL" -f json
opencli youtube search "tutorial" -f json
```
...
> ⚠️ **OpenCLI 通过 Browser Bridge 扩展复用你现有 Chrome 登录态**，无需独立 Chrome 实例或 profile 同步。详见 `references/opencli-integration.md`。

### OpenCLI 浏览器自动化命令（谨慎使用）

OpenCLI 也有 `opencli browser <session> open/click/fill/eval` 等浏览器自动化命令，但**与 Hermes 内置 browser 工具高度重叠**。仅在 Hermes 内置工具不可用或需要复用登录态时使用。
...
│   ├── 平台有 OpenCLI site adapter？
│   │   → terminal("opencli <site> <command> -f json") ← 首选！
...
│   → 需要登录态？用 opencli browser <session> open <url> 代替 browser_navigate
│
└── 桌面应用控制
    → terminal("opencli <app> <command>")
       Cursor/ChatGPT/Codex/Discord 等 Electron 应用
```
...
> `references/opencli-integration.md` — OpenCLI 架构、100+ 站点适配器、与 Hermes 内置工具的关系矩阵。
...
4. **🚫 将 OpenCLI site CLI 仅当作「备用方案」** → 当目标平台有 Site Adapter 时，`opencli <site>` 应**优先于** Hermes browser_* 工具，因为一条命令直接出 JSON 远比逐步 navigate→snapshot→click 高效。仅在无 adapter 或需要精细步骤控制时才降级到 Hermes browser_*。
...
9. **OpenCLI "Extension not connected"** → 确保 Browser Bridge 扩展已安装且启用，运行 `opencli doctor` 诊断
10. **OpenCLI 返回空数据或 "Unauthorized"** → Chrome 登录态可能过期，在 Chrome 中重新登录目标网站
11. **OpenCLI daemon 启动失败** → 检查端口 19825 是否被占用，`curl localhost:19825/status` 查看状态
...
> **与 SOUL.md 的关系**：SOUL.md 不再定义具体规则，仅做强制委托——"工具选择必须严格遵守 tool-selection-strategy 和 OpenCLI Skill"。本 Skill 是工具选择策略的**唯一权威来源**。
